...
#include "ui_StripSlider.h"
#include<QPainter>
#include <QDebug>
#include <QPalette>
#include<QMouseEvent>
#include<math.h>
StripSlider::StripSlider(QWidget *parent)
	: QWidget(parent)
{
	ui = new Ui::StripSlider;
	ui->setupUi(this);
...
StripSlider::~StripSlider()
{
	delete ui;
}

void StripSlider::SetSliveWidth(int value)
{
	this->slive_width = value;
}

void StripSlider::SetBackgrondColor(QColor slive_color, QColor slider_color)
{
	this->slive_color = slive_color;
	this->slider_color = slider_color;
}

void StripSlider::SetTopAndBottomBlank(int top_value, int bottom_value)
{
	this->top_value = top_value;
	this->bottom_value = bottom_value;
}

void StripSlider::SetSliderWidgetWidthAndHeight(int width, int height)
{
	this->wid_width = width;
	this->wid_height = height;
}

void StripSlider::SetSliderRange(int min_value, int max_value)
{
...
void StripSlider::SetSliveRoundValue(int x_value, int y_value)
{
...
void StripSlider::SetHanderLabelProperty(QFont font, QColor font_color, QColor back_color)
{
...
void StripSlider::SetHanderLabelRoundValue(int border_x_value, int border_y_value)
{
...
void StripSlider::Update()
{
	//固定下来的x轴
	hander_x_axis = (m_width - wid_width) / 2;
	//y轴移动的总距离(多减去一个滑块高度)
	hander_y_dis = m_height - top_value - bottom_value- wid_height;
	//y轴的步长
...
void StripSlider::SetSliderValue(int value)
{
...
void StripSlider::resizeEvent(QResizeEvent* size)
{
...
void StripSlider::paintEvent(QPaintEvent* event)
{
...
void StripSlider::MoveSlider(int value)
{
	//当前value值对应的y值
	int cur_y = top_value+(value - min_value)*haner_y_step;
	hander_lab->move(hander_x_axis, cur_y);
	hander_lab->SetText(QString::number(value));
}

bool StripSlider::eventFilter(QObject *obj, QEvent *event)
{
...
void StripSlider::leaveEvent(QEvent* event)
{
...
void StripSlider::mouseMoveEvent(QMouseEvent* event)
{
	if (event->buttons()&Qt::LeftButton)
	{
		...
		case Checked_State:
		{
			if (event->y()<=top_value+wid_height/2)
			{
				cur_value = min_value;
				hander_lab->move(hander_x_axis, top_value);
			...
			else if (event->y() >= m_height-bottom_value-wid_height/2)
			{
				cur_value = max_value;
				hander_lab->move(hander_x_axis, m_height - bottom_value-wid_height);
				hander_lab->SetText(QString::number(cur_value));
			...
			float move = (event->y()-top_value- wid_height/2)/haner_y_step;
