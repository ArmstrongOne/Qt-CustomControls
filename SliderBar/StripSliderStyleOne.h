//这个滚动条控件两头要有方块，我之前为了灵活化无论什么参数都会尽量开成接口从外面传，导致用一个控件传十几个参数，我决定把大众化的样式确定下来

//像这个控件 只需传入一个宽和top-margin bottom-margin所有的参数都能得出来和原始的stripslider一样的接口 不变

//然后方块上的箭头钉死为高是方块1/3 宽是1/2
#pragma once
#include <QLabel>
#include<QPainter>
#include <QColor>
...
namespace Ui
{
	class StripSliderStyleOne;
}

class SLIDERBAR_EXPORT StripSliderStyleOne : public QWidget
{
	Q_OBJECT

public:
	StripSliderStyleOne(QWidget *parent = Q_NULLPTR);
	~StripSliderStyleOne();
	/*****************必要初始化接口*****************/
	//-----------------------滚动条主体设置-----------------------
	//设置滚动条，杆子的宽度
