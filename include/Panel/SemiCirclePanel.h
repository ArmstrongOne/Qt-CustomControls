...
public:
	...
	void SetStartAngle(int start_angle = 45);//设置开始角度(例:6点钟方位为0度 顺针转60度)
	void SetEndAngle(int end_angle = 45);//设置结束角度(例:6点钟方位为0度 逆针转60度)
...
private:
	...
	int out_cir_x;//外圆左上角x坐标
	int out_cir_y;//外圆左上角y坐标

	int out_cir_center_x;//外圆中心点x坐标
	int out_cir_center_y;//外圆中心点y坐标
